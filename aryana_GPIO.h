...
void aryana_GPIOx_Enable(char PortName) // example : GPIOx_Enable('A');  -> enables PortA's clock
{
    switch(PortName)
    {
        case 'A':
...
void aryana_GPIOx_PINiSetMode_INPUT(char PortName,unsigned int pin)
{
    unsigned int pin_Bite_L=1U;
    unsigned int pin_Bite_H;
    
    for(int i = 1; i<=(2*pin) ;i++){
...
void aryana_GPIOx_PINi_High(char PortName,unsigned int pin) // example : GPIOx_PINi_High('A',2) -> sets PA2 to High
{
...
void aryana_GPIOx_PINi_Low(char PortName,unsigned int pin) // example : GPIOx_PINi_LOW('A',2) -> sets PA2 to low ,that is, it resets PA2
{
	unsigned int pinCode=(1U<<0);
...
void aryana_GPIOx_PINiSetMode_OUT(char PortName,unsigned int pin)  // example : GPIOx_PINiSetMode_OUT('A',0);  -> sets PA0 to output
{
    unsigned int pin_Bite_L=1U;
    unsigned int pin_Bite_H;
    for(int i = 1; i<=(2*pin) ;i++){
...
void aryana_GPIOx_PINi_Write(char PortName,unsigned int pin,int bit) // example : GPIOx_PINi_LOW('A',2) -> sets PA2 to low ,that is, it resets PA2
{
     if (bit==0){
        aryana_GPIOx_PINi_Low(PortName,pin);
     }
     else if(bit==1){
        aryana_GPIOx_PINi_High(PortName,pin);
     }
	
}

void aryana_GPIOx_Read(char PortName,int* PortInputValue_Array)
{
